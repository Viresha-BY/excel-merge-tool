...
CSV_SUFFIXES = ['DCA Data', 'DCB Data', 'DCC Data', 'DCD Data']
...
def get_csv_bases_and_suffixes(df):
...
def style_dataframe(df):
    ...
    base_to_cols = get_csv_bases_and_suffixes(df)
    # CSV value consistency: per row, per base, all csv fields with same base must match
    ...
    def validate_cell(col, val, row, row_idx):
    ...
    def style_func(row):
        ...
        for i, col in enumerate(columns):
            val = row.get(col, "")
            valid = validate_cell(col, val, row, row_idx)
            if valid == "duplicate":
